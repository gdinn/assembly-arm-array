...
{	
	...
	cmd_LCD(0xB2);			//Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark
...
uint8_t round_up_divide(uint8_t x, uint8_t y){
	return (x % y) ? x / y + 1 : x / y;
}

int old_move_fig_dir(struct  pontos_t *p, const struct figura_t *f){
	/* Retornos
	 * -1 é o retorno de ilegalidade,
	 *  0 de colisão,
	 *  1 de sucesso
	 */
	...
	int sucesso = SUCESSO_MOVE_FIG;

	if(x>83) { //Como o ponto X ja está fora, tem nem como imprimir
		sucesso = FORA_TELA_MOVE_FIG;
		return sucesso;
	}

	if (y>47) { //imprime pela metade mas blz
		sucesso = COLISAO_MOVE_FIG;
		y = 47;
	}

	//checar o limite esquerdo da figura

	i = x + (84*(y/8));		/* determina��o do indice do byte a ser alterado [0 - 503]	*/


	//retorno = !tst_bit(fb[i]);
	...
	for(j=0; j<alt; j++)
	{
		for(i=0; i<larg; i++)
		{
			if(tst_bit(f->pixels[i + ((j/8)*f->largura)], j%8)){
				sucesso = COLISAO_MOVE_FIG;
			}
	...
	//Faz a figura deslocada
	struct figura_t* fig_deslocada;
	unsigned char* fig_deslocada_pixels;
	fig_deslocada->altura = f->altura;
	fig_deslocada->largura = f->largura + 1; //deslocando p/ direita, entao adiciona a coluna no fim

	uint8_t n_colunas = fig_deslocada->altura / 8;//round_up_divide(fig_deslocada->altura, 8);
	printf("N colunas: %d \n", n_colunas);
	printf("fig largura: %d \n", fig_deslocada->largura);
	n_colunas = n_colunas * fig_deslocada->largura;

	fig_deslocada_pixels = malloc(n_colunas*sizeof(unsigned char));

	for(i=0; i<n_colunas; i=i+fig_deslocada->largura){
		fig_deslocada_pixels[i] = 0b00000000;
	}

	uint8_t n_colunas_old = f->altura/8; //round_up_divide(f->altura, 8);
	n_colunas_old = n_colunas_old * f->largura;

	uint8_t xa = 23;

	memcpy(fig_deslocada_pixels + 1, f->pixels, n_colunas_old*sizeof(unsigned char));

	//FIM da contrucao da figura deslocada


	unsigned char* figura_xor_pixels;
	struct figura_t* figura_xor;
	figura_xor->altura = f->altura;
	figura_xor->largura = f->largura + 1; //deslocando p/ direita, entao adiciona a coluna no fim
	//reserva memoria para a figura xor
	figura_xor_pixels = malloc(n_colunas * sizeof(unsigned char));


	//Faz o xor da figura deslocada com a nao deslocada
	for(i=0; i<n_colunas; i++) {
		if(fig_deslocada_pixels){
			figura_xor_pixels[i] = fig_deslocada_pixels[i] ^ f->pixels[i];
			//resolve o problema de duplo encontro, mas para bordas da deslocada n
			figura_xor_pixels[i] = figura_xor_pixels[i] & fig_deslocada_pixels[i]; //ver logica

				//Verificar se tem uma funcao apaga pixel q só apaga onde é 1 e onde é 0 deixa como está
		}
	}

	//FIM do xor da figura deslocada com a nao deslocada

	figura_xor->altura = fig_deslocada->altura;
	figura_xor->largura = fig_deslocada->largura;
	memcpy(figura_xor->pixels, figura_xor_pixels, n_colunas*sizeof(unsigned char));



	struct pontos_t p_xor;
	p_xor.x1 = x;
	p_xor.y1 = y;
	...
	desenha_fig(&p_xor, figura_xor);

/*
	for(j=0; j<alt; j++) //escrita xor para apagar os rastros da imagem
	{
		for(i=0; i<larg; i++)
		{
			//checar se o bit é 1 e escrever 0
			if(tst_bit(figura_xor_pixels[i + ((j/8)*figura_xor->largura)], j%8)){
...
int move_fig_esq(struct  pontos_t *p, const struct figura_t *f, int pixels_mover) {
	desenha_fig_apagar(p, f);
	p->x1 = p->x1 - pixels_mover;
	if(p->x1 < 0){
		if(abs(p->x1) < f->largura) {
			p->x3 = abs(p->x1);
			p->y3 = 0;
			p->dir3 = 1;
		} else {
...
void move_fig_dir(struct  pontos_t *p, const struct figura_t *f, uint8_t pixels_mover) {
	desenha_fig_apagar(p, f);
	p->x1 = p->x1 + pixels_mover;
	desenha_fig(p, f);
}

void move_fig_cima(struct  pontos_t *p, const struct figura_t *f, uint8_t pixels_mover) {
	desenha_fig_apagar(p, f);
	p->y1 = p->y1 + pixels_mover;
	desenha_fig(p, f);
}

void move_fig_baixo(struct  pontos_t *p, const struct figura_t *f, uint8_t pixels_mover) {
	desenha_fig_apagar(p, f);
	p->y1 = p->y1 - pixels_mover;
	desenha_fig(p, f);
}
				...
				const struct figura_t *f)	// ponteiro para a figura definido pelo tipo figura_t
{
	...
	if(((p->x3!=0) || (p->y3!=0)) && (p->dir3 == 1))
	{
		if(p->dir3 == 1){
			...
			uint8_t gravar = 0;
			uint8_t indice_fig = 0;


			for(j=0; j<(f->altura - p->y3); j++)
			{
				for(i=0; i<(f->largura - p->x3); i++) // canto inferior direito
				{
					indice_fig = i + ajx + (((j+ajy)/8)*f->largura);
					gravar = tst_bit(f->pixels[indice_fig], (j+ajy)%8);
					desenha_pixel(x+i, y+j, gravar);
				}
			}
...
void desenha_fig_apagar(struct  pontos_t *p,		// [p.x1 p.y1] para o ponto P1 de incersao da figura
											// Caso se deseje alterar a largura e altura da figura
											//    p.x2  e  p.y2 devem ser diferentes de zero, determinando as novas medidas
											// Caso se deseje imprimir a partir de um ponto no meio da figura (se n�o empregados devem ser zero)
											//    p.x3 e p.y3 determinam esse ponto
