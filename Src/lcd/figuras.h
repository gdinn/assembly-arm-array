#ifndef FIGURAS_H_
#define FIGURAS_H_

//504 bytes por figura - 48x84 pixels


/*
	struct pontos_t p;
	p.x1 = 4;
	p.y1 = 4;
	p.x2 = 0;
	p.y2 = 0;
	p.x3 = 0;
	p.y3 = 0;  
	desenha_fig(&p, &igao);
*/
const struct figura_t igao = {
		4,							// largura em pixels = numero de colunas da matriz
		8,							// altura em pixels = cada 8 pixels corresponte a uma linha da matriz
		{
			0b11111111, 0b11111111, 0b11111111, 0b11111111
		 }
};

const struct figura_t boca_torre = {
		12,							// largura em pixels = numero de colunas da matriz
		2,							// altura em pixels = cada 8 pixels corresponte a uma linha da matriz
		{
			0b11, 0b11, 0b11, 0b11, 0b11, 0b11, 0b11, 0b11, 0b11, 0b11, 0b11, 0b11,
		 }
};

const struct figura_t boca_torre_apagar = {
		12,							// largura em pixels = numero de colunas da matriz
		2,							// altura em pixels = cada 8 pixels corresponte a uma linha da matriz
		{
			0b00, 0b00, 0b00, 0b00, 0b00, 0b00, 0b00, 0b00, 0b00, 0b00, 0b00, 0b00,
		 }
};

const struct figura_t corpo_torre = {
		10,							// largura em pixels = numero de colunas da matriz
		14,							// altura em pixels = cada 8 pixels corresponte a uma linha da matriz
		{
			0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111,
			0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111
		 }
};

const struct figura_t bichano = {
		8,							// largura em pixels = numero de colunas da matriz
		8,							// altura em pixels = cada 8 pixels corresponte a uma linha da matriz
		{
			0b00111100, 0b01111110, 0b11111111, 0b11001111, 0b11100111, 0b11001111, 0b01111110, 0b00111100
		 }
};

const struct figura_t corpo_torre_apagar = {
		10,							// largura em pixels = numero de colunas da matriz
		14,							// altura em pixels = cada 8 pixels corresponte a uma linha da matriz
		{
			0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000,
			0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000
		 }
};

const struct figura_t torre_baixo = {
		12,							// largura em pixels = numero de colunas da matriz
		16,							// altura em pixels = cada 8 pixels corresponte a uma linha da matriz
		{
			0b00000011, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b00000011,
			0b00000000, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b11111111, 0b00000000
		 }
};

const unsigned char dragon [] = {
	,
};
// ---------------------------------------------------------------------------------------
// Os pixeis n�o empregados na definic�o dos bytes DEVEM ser ZERO, ou seja, os pixeis que n�o devem ser impressos
// A figura � desenhada conforme trabalho do LCD, do LSB to MSB (bytes alinhados verticalmente, ver manual do LCD5110)
//-----------------------------------------------------------------------------------------------------------------
const struct figura_t fig1 =
{
	4,							// largura em pixels = numero de colunas da matriz
	24,							// altura em pixels = cada 8 pixels corresponte a uma linha da matriz
	{0b11111111, 0b10000011, 0b10000011, 0b11111111,
	 0b11111110, 0b11000001, 0b11000001, 0b11111110,
	 0b11111110, 0b11000001, 0b11000001, 0b11111110,}
};


#endif /* FIGURAS_H_ */
